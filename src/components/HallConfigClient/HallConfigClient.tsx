...
export function HallConfigClient() {
    ...
    const handleClickSeat = (rowIndex: number, seatIndex: number, seatType: string) => {
        if (seatType === 'taken' || seatType === 'disabled') {
            return;
        }
    ...
    const handleBuyTickets = async () => {
        const tickets = selectedSeats.map((seat) => {
            ...
            const getSeatPrice = (currentSeatType: string): number => {
                if (!hall) {
                    throw new Error('Hall data is not available');
                }

                if (currentSeatType === 'vip') {
                    return hall.hall_price_vip;
                }

                if (currentSeatType === 'standart') {
                    return hall.hall_price_standart;
                }

                throw new Error(`Unknown seat type: ${currentSeatType}`);
            };

            return {
                row: rowIndex + 1,
                place: seatIndex + 1,
                coast: getSeatPrice(seatType),
            };
        });
        ...
        if (tickets.length > 0 && seanceId && date) {
            try {
                ...
                if (result.success && result.result) {
                    ...
                    const selectedSeatsInfo = tickets.map((ticket) => `Ряд ${ticket.row} - место ${ticket.place}`).join(', ');
                    ...
                    navigate('/payment', {
                        state: {
                            ...
                            ticketRows: tickets.map((ticket) => ticket.row).join(', '),
                            ticketPlaces: tickets.map((ticket) => ticket.place).join(', '),
            ...
            <div className={styles.container}>
                <div className={styles.seance}>
                    ...
                    <div className={styles.tapHint}>
                        <img
                            src={assetPath("Client/hint.svg")}
                            alt=""
                            aria-hidden="true"
                            className={styles.tapHintIcon}
                        />
                        <p>
                            Тапните
                            <br />
