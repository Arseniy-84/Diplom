...
export const DeleteSeance = ({ onSeanceDelete }: DeleteSeanceProps) => {
    ...
    return (
        <div ref={drop as any} className={`${styles.trashBin} ${isOver ? styles["drag-over"] : ""}`}>
            <img src={assetPath("Admin/trash-icon.svg")} alt="Иконка мусорной корзины" className={styles.icon} />
        </div>
    );
};
