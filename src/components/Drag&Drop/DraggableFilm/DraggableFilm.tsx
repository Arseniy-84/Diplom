...
export const DraggableFilm = ({ film, backgroundColor, onDelete }: DraggableFilmProps) => {
    ...
    return (
        <div ref={drag as any} style={{ backgroundColor }} className={`${styles.film} ${isDragging ? styles.dragging : ""}`}>
            ...
            <img src={assetPath("Admin/delete-icon.svg")} alt="иконка удаления" className={styles.icon} onClick={() => onDelete(film.id)} />
        </div>
    );
};
